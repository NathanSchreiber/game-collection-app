    ...
    def __str__(self):
        ...
        for game in self.games:
            ...
            if entry.rating == None:
                print_game = f"Title: {entry.title}, Platform: {entry.platform}, Finished: {entry.finished}, Playtime: {entry.playtime}, ID: {entry.id}"
                print_game_list += f"{print_game}\n"
            else:
                print_game = f"Title: {entry.title}, Platform: {entry.platform}, Finished: {entry.finished}, Rating: {entry.rating}/10 Playtime: {entry.playtime}, ID: {entry.id}"
                print_game_list += f"{print_game}\n"
